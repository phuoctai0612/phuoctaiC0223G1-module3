...
hd.ngay_lam_hop_dong,hd.ngay_ket_thuc,sum(dv.chi_phi_thue*hdct.so_luong+dv.chi_phi_thue) as "tổng tiền"
from khach_hang kh
...
left join dich_vu_di_kem dvdk
 ...
 select hd.ma_hop_dong,nv.ho_va_ten,kh.ho_ten,kh.so_dien_thoai,dv.ma_dich_vu,dv.ten_dich_vu,sum(dvdk.ma_dich_vu_di_kem) ,hd.tien_dat_coc
 from hop_dong hd
join nhan_vien nv
 ...
 on dvdk.ma_dich_vu_di_kem=hdct.ma_dich_vu_di_kem
 where (month(hd.ngay_lam_hop_dong) between 9 and 12)
 group by hd.ma_hop_dong;
 ...
 select nv.*,count(hd.ma_nhan_vien)
 from nhan_vien nv
 join hop_dong hd
...
where nhan_vien.ma_nhan_vien not in (select nv.ma_nhan_vien,count(hd.ma_nhan_vien)
 from nhan_vien nv
 join hop_dong hd
 on nv.ma_nhan_vien=hd.ma_nhan_vien
 group by nv.ma_nhan_vien
